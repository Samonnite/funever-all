import { AdminGameEdit, Game } from 'api/model';
import { yupResolver } from '@hookform/resolvers/yup';
import { useForm } from 'react-hook-form';
import * as yup from 'yup';
import { useQueryParams } from 'hooks/useQueryParams';
import { useGameQuery, useGameEdit, useGameDelete } from 'api';
...
export const useGameUpdateForm = (defaultValue?: Game) => {
  const form = useForm<any>({
    ...
    resolver: yupResolver(
      yup.object().shape({
        ...
        rank: yup.number().typeError('请输入数字'),
      })
    ),
  });
  ...
  useEffect(() => {
    if (defaultValue) {
      ...
      setValue('rank', defaultValue?.rank);
      setValue('mates.logo', defaultValue?.mates?.logo);
      setValue('mates.tags', defaultValue?.mates?.tags);
      setValue('mates.banner', defaultValue?.mates?.banner);
    }
...
export const GameModel = (() => ({
  ...
  useDeleteGame() {
    const { refetch } = this.useGameList();
    const [query] = useQueryParams();
    const { mutateAsync: deleteGame, ...result } = useGameDelete({
      mutation: {
        onSuccess() {
          refetch(query);
        },
