import { Grid, IconButton, Paper } from '@mui/material';
import FormProvider, { RHFQueryAutocomplete, RHFTextField } from 'components/hook-form';
import { LoadingButton } from '@mui/lab';
import { GameQuery } from 'api/model';
import Iconify from 'components/iconify';
import { GameModel } from 'pages/game/model';
import usePaginationAndSearch from 'hooks/usePaginationAndSearch';
import { useContestQueryForm } from '../model/index';

const QueryForm: React.FC<{
  refetch: (values: GameQuery) => void;
}> = ({ refetch }) => {
  const methods = useContestQueryForm();
  const { gameList, refetch: refetchGame } = GameModel.useGameList();
  usePaginationAndSearch({ refetch: refetchGame, isMain: false });
  const { reset, handleSubmit } = methods;

  return (
    <FormProvider methods={methods} onSubmit={handleSubmit(refetch)}>
      <Paper sx={{ my: 2 }}>
        <Grid container spacing={2}>
          <Grid item>
            <RHFTextField name="keyword" label="搜索赛事名称" sx={{ width: 180 }} />
          </Grid>
          <Grid item>
            <RHFQueryAutocomplete
              name="gid"
              label="所属游戏"
              lableKey="id"
              field="id"
              options={gameList}
              getOptionLabel={(id: any) => {
                const option = gameList?.find((item) => item.id === id);
                if (option) {
                  return `${option?.name}`;
                }
                return '';
              }}
              sx={{ width: 180 }}
              handleChange={(values) => refetch(values)}
            />
          </Grid>
          <Grid item>
            <LoadingButton variant="contained" type="submit" sx={{ mr: 1 }}>
              查询
            </LoadingButton>
            <IconButton
              onClick={() => {
                reset();
                refetch({});
              }}
              sx={{ ml: 1 }}
            >
              <Iconify icon="carbon:reset" />
            </IconButton>
          </Grid>
        </Grid>
      </Paper>
    </FormProvider>
  );
};

export default QueryForm;
